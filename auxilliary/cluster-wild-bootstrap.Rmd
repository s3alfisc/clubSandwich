...
3. For $b = 1,...,B$:
    a. Calculate the full vector of perturbed residuals as $\mathbf{e}^{(b)} = \left(\eta_1\mathbf{f}_1',...,\eta_m\mathbf{f}_m'\right)'$. 
    b. Adjust the perturbed residuals for the null model by taking $\mathbf{\tilde{e}}^{(b)} = \left(\mathbf{I} - \mathbf{H_T}\right) \mathbf{e}^{(b)}$. Let $\mathbf{\tilde{e}}^{(b)}_j$ denote the sub-vector of $\mathbf{\tilde{e}}^{(b)}$ corresponding to cluster $j$.
    c. Calculate $\boldsymbol{\hat\beta}^{(b)} = \mathbf{M_{\ddot{U}}} \sum_{j=1}^m \mathbf{E}_j \mathbf{\tilde{e}}_j^{(b)}$.
    d. Calculate $\mathbf{\hat{e}}_j^{(b)} = \mathbf{e}_j^{(b)} - \mathbf{\ddot{U}}_j \boldsymbol{\hat\beta}^{(b)}$.
    e. Calculate $\mathbf{V}^{(b)} = \mathbf{M_{\ddot{U}}}\left(\sum_{j=1}^m \mathbf{G}_j'\mathbf{\hat{e}}_j^{(b)} \left(\mathbf{\hat{e}}_j^{(b)}\right)' \mathbf{G}_j\right)\mathbf{M_{\ddot{U}}}$
    f. Calculate $Q^{(b)} = \left(\boldsymbol{\hat\beta}^{(b)}\right)' \left(\mathbf{V}^{(b)}\right)^{-1} \boldsymbol{\hat\beta}^{(b)}$.
