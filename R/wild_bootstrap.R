...
Wild_bootstrap <- function(obj, constraints,  
  ...
  U_matrx <- model.matrix(data = data, object =  constraints) # question: add an argument called "data"?
  ...
  if(UseWeight == T){
    ...
    W_vector <- rep(x = 1,nrow(T_matrx)) # No weights. All 1s.
    W_matrx <- diag(x = W_vector, nrow = length(W_vector))
  ...
  obj_fullmodel <- list(study_orig_id = cluster,
                        b.r = T,
                        data.full = list(e.r = e_hat,
                                         userweights = W_vector),
                        Xreg = cbind(U_matrx,T_matrx),
                        user_weighting = T,
                        N = length(table(cluster))
  )
  
  class(obj_fullmodel) <- "supplemented_WLS"
  
  vcov_full <- vcovCR(obj = obj_fullmodel, 
                      cluster = cluster, 
                      type = test_adjustment, 
                      inverse_var = attr(vcov_null, "inverse_var"),
                      target = attr(vcov_null, "target"))
